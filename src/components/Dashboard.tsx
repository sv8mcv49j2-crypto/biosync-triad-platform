...
const Dashboard = ({ onTelehealth, userData }: DashboardProps) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'appointments' | 'protocols' | 'library' | 'insights'>('overview');
          ...
          {isPremium && (
            <>
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-2 rounded-lg font-medium transition-colors ${activeTab === 'insights' ? 'bg-purple-50 text-purple-700' : 'text-gray-600 hover:bg-gray-50'}`}
              >
                <TrendingUp size={20} />
